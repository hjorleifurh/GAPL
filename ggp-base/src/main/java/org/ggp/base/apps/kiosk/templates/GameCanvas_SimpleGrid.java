...
public abstract class GameCanvas_SimpleGrid extends GameCanvas {
    ...
    protected Dimension getCellDimension() {
        ...
        int nCellWidth = width / getGridWidth();
        int nCellHeight = height / getGridHeight();
        // make cells squares
        nCellWidth = Math.min(nCellHeight, nCellWidth);
        return new Dimension(nCellWidth, nCellWidth);
    ...
    protected final void paintGame(Graphics g) {
        ...
        for(int x = 0; x < nGridWidth; x++) {
            for(int y = 0; y < nGridHeight; y++) {
                Graphics cellGraphics = g.create(x*cellDimension.width, y*cellDimension.height, cellDimension.width, cellDimension.height);
    ...
    protected final void handleClickEvent(int x, int y) {
        ...
        int xCell = x / cellDimension.width;
        int yCell = y / cellDimension.height;

        int xWithin = x % cellDimension.width;
        int yWithin = y % cellDimension.height;

        // System.out.println("clicked on " + x + "," + y + " -> cell: " + xCell + "," + yCell + "(cellSize: " + cellDimension + ")");
        if(coordinatesStartAtOne()) {
