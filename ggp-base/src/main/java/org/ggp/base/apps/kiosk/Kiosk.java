...
public final class Kiosk extends JPanel implements ActionListener, ItemListener, Observer
{
    ...
    public Kiosk()
    {
        ...
        computerAddress = new JTextField("localhost:9147");

        gamers = Lists.newArrayList(ProjectSearcher.GAMERS.getConcreteClasses());
        List<Pair<Class<? extends Gamer>, String>> gamersAndNames = new ArrayList<>(gamers.size());
        for(Class<? extends Gamer> gamer : gamers) {
            try {
                Gamer g = (Gamer) gamer.newInstance();
                if (!g.isComputerPlayer()) {
                    // Kiosk only considers computer players
                	continue;
                }
                gamersAndNames.add(Pair. <Class<? extends Gamer>,String> of(gamer, g.getName()));
            } catch(Exception ex) {
            }
        }
        Collections.sort(gamersAndNames, new Comparator<Pair<?, String>>() {
				@Override
        ...
        gamers.clear();
        List<String> gamerNames = new ArrayList<String>(gamersAndNames.size());
        for (Pair<Class<? extends Gamer>, String> pair : gamersAndNames) {
        	gamers.add(pair.left);
        	gamerNames.add(pair.right);
        }
        gamerNames.add(remotePlayerString);

        playerComboBox = new JComboBox<String>(gamerNames.toArray(new String[]{}));
